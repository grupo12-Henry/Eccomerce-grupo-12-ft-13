...
const router = Router();
router.get("/pedidos/filter", async (req, res) => {
  //envia todos los pedidos con el estado especificado
...
router.get("/productos/names", async (req, res) => {
  //envia todos los nombres de los productos
  try {
...
router.get("/productos/id/:id", async (req, res) => {
  //devuelve el producto con determinado id
  const id = req.params.id;
  try {
...
router.put("/productos/:id", async (req, res) => {
  //modifica el producto seleccionado mediante id
  const id = req.params.id;
  const { stock, name, type, Description, price, image, maker, subcategories } =
    req.body;
  try {
    const product = await Product.findByPk(id);
...
router.get("/users/id/:id", async (req, res) => {
  //trae usuario con todos sus pedidos, datos de envios, facturas
  const id = req.params.id;
  try {
  ...
  try {
    ...
    products.forEach((e) => {
      newOrder.setProducts(e.id, {
        through: { cantidad: e.cantidad, subTotal: e.subtotal },
      });
    });
...
router.put("/pedidos/id/:id", async (req, res) => {
  //modifica un pedido segun los datos enviados(no hace falta enviar todos los campos)
  const id = req.params.id;
  const {
    bill,
    date,
...
router.put("/users/:id", async (req, res) => {
  ...
  const { name, phone, lastName, state, adress, mail, identityCard, admin } =
    req.body;
  try {
    const user = await Client.findByPk(id);
